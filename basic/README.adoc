...
<1> By extending the Spring Data Commons' `CrudRepository` we get a lot of functionality for free; we just have to plug in the type of the domain object (`Employee`) and its primary key (`Long`).
====

That is all that is needed! In fact, you need not even annotate the interface if it is top-level and visible. If you use your IDE and open up `CrudRepository`, you will find a collection of pre-defined methods.
...
The last step needed to get a fully operational REST API off the ground is to have a `public static void main` method that launches Spring Boot, as follows:

NOTE: If you created your project with https://start.spring.io, a class with this method has been generated for you.
...
Assuming the previous class as well as your Maven build file were generated from https://start.spring.io, you can now launch the app either by running that `main()` method inside your IDE or by typing `./mvnw spring-boot:run` on the command line. (`mvnw.bat` for Windows users.)
...
This little plugin performs multiple steps:
...
* The `webpack` command will execute the webpack binary, which compiles all the JavaScript code based on `webpack.config.js`.

These steps are run in sequence, essentially installing node.js, downloading JavaScript modules, and building the JS bits.

What modules are installed? JavaScript developers typically use `npm` to build up a `package.json` file, such as the following; in our case, you'll create it in the root of your project. 
...
* Compiles ALL of the JavaScript bits into `./src/main/resources/static/built/bundle.js`, which is a JavaScript equivalent to a Spring Boot uber JAR. All your custom code AND the modules pulled in by the `require()` calls are stuffed into this file.
...
Using JavaScript's map function, `this.props.employees` is transformed from an array of employee records into an array of `<Employee />` React components (which you will see a little later).
